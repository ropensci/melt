test_that("invalid 'object", {
  ...
  expect_error(lht2(fit, lhs = 1, control = optcfg))
  x <- rnorm(10)
  y <- 1 + x + rnorm(10)
  df <- data.frame(x, y)
  fit <- el_lm2(y ~ 0., df)
...
test_that("invalid 'lhs' and 'rhs'", {
  ...
  expect_error(lht2(fit, control = optcfg))
  expect_error(lht2(fit, rhs = c(NA, 0), lhs = lhs, control = optcfg))
  expect_error(lht2(fit, rhs = c(1, 0), lhs = lhs, control = optcfg))

  w <- 1 + runif(n, min = -0.5, max = 0.5)
  fit2 <- el_lm2(y ~ x + x2, df, weights = w, control = optcfg)
  out <- lht2(fit2, lhs = lhs, control = optcfg)
  expect_lt(out@statistic, 20)

  # invalid lhs
  lhs <- matrix(c(1, 0, 0, 0, NA, 1), nrow = 2)
  expect_error(lht2(fit, lhs = lhs, control = optcfg))
  expect_error(lht2(fit2, lhs = lhs, control = optcfg))
...
test_that("when lht == eval", {
  ...
  fit2 <- lht2(fit, rhs = par, control = optcfg)
  ...
  rhs <- c(0, 0)
  fit2 <- lht2(fit, lhs = lhs, rhs = rhs, control = optcfg)
...
test_that("invalid 'control", {
  ...
  par <- runif(1, min(x), max(x))
  fit <- el_mean2(par, x)
  expect_error(lht2(fit, lhs = 1, control = list(maxit = 200L)))
})
