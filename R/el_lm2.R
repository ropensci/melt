...
el_lm2 <- function(formula, data, weights = NULL, na.action,
  ...
  if (is.empty.model(mt)) {
    ...
    return(new("LM",
      ...
      weights = check_weights(w, nrow(mm)),
  ...
  new("LM",
    optim = el$optim, logp = el$log.prob, logl = el$loglik, loglr = el$loglr,
    statistic = el$statistic, df = df, pval = pval, npar = p, weights = w,
    dataMatrix = if (model) mm else matrix(NA_real_, nrow = 0L, ncol = 0L),
    coefficients = z$coefficients, parTests = el$par.tests,
    na.action = attr(mf, "na.action"), xlevels = .getXlevels(mt, mf), call = cl,
    terms = mt
  )
}
